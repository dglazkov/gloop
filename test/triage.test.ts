...
test("planTriage: priority maps to a label op", () => {
	const plan = planTriage([{ issue: 1, priority: "high" }], [issue(1)]);
...
test("planTriage: replacing an existing priority removes the old label", () => {
	const plan = planTriage([{ issue: 1, priority: "critical" }], [issue(1, ["priority:low", "bug"])]);
...
test("planTriage: recognizes P0-P3 spellings as priority labels to replace", () => {
	const plan = planTriage([{ issue: 1, priority: "medium" }], [issue(1, ["P1"])]);
...
test("planTriage: priority already correct is a no-op", () => {
	const plan = planTriage([{ issue: 1, priority: "high" }], [issue(1, ["priority:high"])]);
	assert.deepEqual(plan.ops, []);
	assert.deepEqual(plan.skipped, []);
});

test("planTriage: duplicate maps to a duplicate op", () => {
	const plan = planTriage([{ issue: 2, duplicateOf: 1 }], [issue(1), issue(2)]);
	assert.deepEqual(plan.ops, [{ kind: "duplicate", issue: 2, of: 1 }]);
});

test("planTriage: duplicate of a non-open issue is skipped", () => {
	const plan = planTriage([{ issue: 2, duplicateOf: 99 }], [issue(1), issue(2)]);
	assert.deepEqual(plan.ops, []);
...
test("planTriage: self-duplicate is skipped", () => {
	const plan = planTriage([{ issue: 2, duplicateOf: 2 }], [issue(2)]);
	assert.deepEqual(plan.ops, []);
	assert.equal(plan.skipped.length, 1);
});

test("planTriage: needsDesign maps to a design op", () => {
	const plan = planTriage([{ issue: 1, needsDesign: true }], [issue(1)]);
	assert.deepEqual(plan.ops, [{ kind: "design", issue: 1 }]);
	assert.deepEqual(plan.skipped, []);
});

test("planTriage: needsDesign is skipped when already routed or awaiting a human", () => {
	for (const label of ["gloop:design", "gloop:epic", "gloop:needs-human"]) {
		const plan = planTriage([{ issue: 1, needsDesign: true }], [issue(1, [label])]);
		assert.deepEqual(plan.ops, [], `expected no ops for ${label}`);
		assert.equal(plan.skipped.length, 1, `expected skip for ${label}`);
		assert.equal(plan.skipped[0].issue, 1);
		assert.match(plan.skipped[0].reason, new RegExp(label));
	}
});

test("planTriage: needsDesign false produces no design op", () => {
	const plan = planTriage([{ issue: 1, needsDesign: false, priority: "low" }], [issue(1)]);
	assert.deepEqual(plan.ops, [{ kind: "label", issue: 1, add: "priority:low", remove: [] }]);
});

test("planTriage: no decompose op exists anymore", () => {
	// The design session owns decomposition; triage entries no longer carry followUps.
	const entries = [{ issue: 1, followUps: [{ title: "x", body: "y" }] }] as unknown as TriageEntry[];
	const plan = planTriage(entries, [issue(1)]);
	assert.deepEqual(plan.ops, []);
});

test("planTriage: unknown issue numbers are skipped", () => {
	const plan = planTriage([{ issue: 42, priority: "low" }], [issue(1)]);
	assert.deepEqual(plan.ops, []);
...
test("planTriage: one entry can carry priority, duplicate, and needsDesign", () => {
	const entries: TriageEntry[] = [{ issue: 2, priority: "medium", duplicateOf: 1, needsDesign: true }];
	const plan = planTriage(entries, [issue(1), issue(2)]);
	assert.deepEqual(
